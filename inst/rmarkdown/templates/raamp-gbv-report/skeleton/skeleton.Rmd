...
generate_minimum_gbv <- function(data) {
  data %>%
...
# logtrans <- "log2"
# common.annual.no.na <- get_common_annual_no_na(common.data)
...
get_data_services <- function(data) {
  get_service_data(
    data,
...
hlth.serv.tbl <- make_ptbl(get_data_services(health.data))
```
...
generate_minimum_gbv(health.data) %>%
...
minimum <- generate_minimum_gbv(health.data) 
minimum %>% 
  convert_to_yn %>% 
...
legal.data.sops <- get_legal_sops(legal.data)
...
judicial.sops <- get_jud_sops(judicial.data)
...
```{r }
training.lawenf <- get_training(security.data, rgx.lawenf.train, names.lawenf.train)
...
confidentiality.data <- get_confidentiality_data(common.data)
...
```{r plot_referrals_from_judiciary_and_legal, anchor = "figure"}
